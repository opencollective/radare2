...
RList *PE_(r_bin_mdmp_pe_get_symbols) (RBin *bin, struct PE_(r_bin_mdmp_pe_bin) * pe_bin) {
	int i;
	ut64 offset;
	...
	if ((symbols = PE_(r_bin_pe_get_exports) (pe_bin->bin))) {
		for (i = 0; !symbols[i].last; i++) {
			...
			ptr->forwarder = r_str_constpool_get (&bin->constpool, (char *)symbols[i].forwarder);
