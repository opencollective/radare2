...
R_API bool r_anal_fcn_insert(RAnal *a, RAnalFunction *fcn) {
	D eprintf ("insert\n");
	if (a->cb.on_fcn_new) {
		a->cb.on_fcn_new (a, a->user, fcn);
	}
	if (a->flg_fcn_set) {
		a->flg_fcn_set (a->flb.f, fcn->name, fcn->addr, r_anal_fcn_size (fcn));
	}
	return r_anal_add_function_ll (a, fcn);
#if 0
...
R_API int r_anal_fcn_bb_overlaps(RAnalFunction *fcn, RAnalBlock *bb) {
	RAnal *a = fcn->anal;
	D eprintf ("bb.overlaps is slow\n");
	RAnalBlock *bbi = r_anal_get_block (a, bb->addr);
	if (bbi) {
